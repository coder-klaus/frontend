...
// 获取输入框输入的值
// 如果不设置定时器，那么inputEl.value的值是空字符串，因为用户还没有输入的时候，界面依旧加载完毕了
// 所以设置对应的定时器或者事件监听，以便于进行测试
setTimeout(() => console.log(inputEl.value), 2000)
```
...
| tagName  | 获取元素的标签名词<br />只有元素才会有标签名<br />如果获取的是非元素节点，属性值为undefined<br />如果获取的是原生节点，属性值为全大写形式表示的元素名 |
