...
如果需要使用非受控组件中的数据，那么就必须要使用 ref 来从DOM节点中获取表单数据

并通过defaultValue或defaultChecked来设置对应的默认值
...
export class App extends PureComponent {
  ...
  componentDidMount() {
    const nameEl = this.nameRef.current

    // 拿到dom元素后，主动监听其对应值的改变
    // 并修改对应defaultValue所绑定的变量值
  ...
  render() {
    ...
    return (
      <div>
        {/*
          非受控组件在绑定默认值的时候
          <select> 和 <textarea> 支 持 defaultValue
          <input type="checkbox"> 和 <input type="radio"> 支持 defaultChecked

          因为受控组件的状态会被统一放置到react的state中进行统一管理
          而非受控组件是交由原生表单元素进行管理的
          所以在实际开发中，推荐使用受控组件，而不是非受控组件
        */}
...
export class App extends PureComponent {
	...
	render() {
		return (
			<div>
				<input type="text" defaultValue={this.state.username} onChange={e => this.handleChange(e)} />
