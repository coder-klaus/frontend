目前，前端最流行的开发方式是，而CSS的设计就不是为组件化而生的，所以在目前组件化的框架中都在需要一种合适的CSS解决方案
...
// ps: 默认情况下对应样式是不会高亮的，需要高亮可以安装vscode-styled-components
...
export default styled.div`
	.title {
		/*
			...
			该回调函数的参数为外部传入的props，返回值是所需要设置的对应的值
		*/
...
但是使用上述写法，每使用一次对应的值就需要单独设置一次对应的默认值，这必然是十分麻烦的

所以styled-components提供了attrs方法，专门用于设置对应的默认值

```jsx
import styled from 'styled-components'

// attrs方法会返回对应的生成样式组件函数
// 所以在这里可以链式调用
// 在attrs中可以传入一个样式对象，用于设置对应的默认值
// 参数是一个回调函数，会将对应的props传递过来
