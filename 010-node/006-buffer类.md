...
比如读取图片，音频或视频数据( Node中有一个Sharp的图片处理库，就是读取图片或者传入图片的Buffer对其再进行处理)

比如在Node中通过TCP建立长连接 (TCP传输的是字节流，我们需要将数据转成字节再进行传入，并且需要知道传输字节的大小(客户端需要根据字节的大小来判断读取多少内容))
