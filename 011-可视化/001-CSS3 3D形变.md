...
CSS3 transform不仅可以实现某个元素的2D形变，也允许我们对某个元素在三维空间中直观的变换元素
...
因此用transform属性旋转或倾斜元素，会变换或倾斜元素的坐标系。并且该元素所有后续变换都将基于新坐标系的变换
...
| 方法                    | 说明 | 描述                                                  | 示例                                                         |
| ----------------------- | ---- | ----------------------------------------------------- | ------------------------------------------------------------ |
| translate3d(tx, ty, tz) | 平移 | translateX(tx) 、translateY(ty)、translateZ(tz)的简写 | translate3d(100px, 100px, 100px)                             |
| scale3d(sx, sy, sz)     | 缩放 | scaleX(sy)、scaleY(sy)、scaleZ(sz)的简写              | scale3d(45deg, 45deg, 45deg)                                 |
| rotate3d(x, y, z, a)    | 旋转 | rotateX(x)、rotateY(y)、rotateZ(z)的简写              | rotate3d(0,1, 0, 45deg)<br />上述示例表示的是y轴顺时针旋转45deg |
...
3D旋转函数定义了一个变换，它将元素围绕固定轴旋转。旋转量由指定的角度确定; 为正，旋转将为顺时针，为负，则为逆时针
...
3D旋转函数有rotateX(deg)、rotateY(deg)、rotateZ(deg)， rotate3d(x, y, z, a)是这三个函数的简写形式
...
transform-style用于设置元素的子元素是定位在 3D 空间中还是平展在元素的2D平面中

transform-style可以控制是否给子元素开启3D空间，所以该属性是设置在要进行3d形变元素的父元素上，这种父元素被称之为舞台或画布

transform-style不是一个可继承属性
...
4. 当前有对于 opacity、transform、fliter、backdrop-filter 应用动画
...
1. 对 opacity、transform、fliter、backdropfilter应用了animation或transition，且是在动画执行中
2. 有 3D transform 函数:比如: translate3d、 translateZ、 scale3d 、 rotate3d ...
3. will-change 设置为 opacity、transform、top、left、bottom、right，fliter，backdropfilter
