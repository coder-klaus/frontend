...
| sessionStorage (会话存储) | 提供的是本次会话的存储，在关闭掉会话时，存储的内容会被清除，更像是一种临时存储机制<br />也就是说当关闭掉网页或者新开一个新的标签页时，sessionStorage中存储的内容会被移除<br />sessionStorage只能在同一个页签的多个同源页面之间进行storage共享 |
