...
Page({
  ...
  async fetchList() {
    try {
      ...
      this.setData({
        homeList: [...this.data.homeList, ...res]
      })
    } catch(e) {
...
[`showLoading`](https://developers.weixin.qq.com/miniprogram/dev/api/ui/interaction/wx.showLoading.html)的功能和showToasd是类似的，其相当于设置了icon的值为loading的toast
...
<navigator url="/pages/index/index" open-type="navigateBack">返回</navigator>
```
